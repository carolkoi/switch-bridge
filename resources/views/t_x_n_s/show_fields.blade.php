<!-- Date Time Added Field -->
<div class="form-group">
    {!! Form::label('date_time_added', 'Date Time Added:') !!}
    <p>{{ $tXN->date_time_added }}</p>
</div>

<!-- Added By Field -->
<div class="form-group">
    {!! Form::label('added_by', 'Added By:') !!}
    <p>{{ $tXN->added_by }}</p>
</div>

<!-- Date Time Modified Field -->
<div class="form-group">
    {!! Form::label('date_time_modified', 'Date Time Modified:') !!}
    <p>{{ $tXN->date_time_modified }}</p>
</div>

<!-- Modified By Field -->
<div class="form-group">
    {!! Form::label('modified_by', 'Modified By:') !!}
    <p>{{ $tXN->modified_by }}</p>
</div>

<!-- Source Ip Field -->
<div class="form-group">
    {!! Form::label('source_ip', 'Source Ip:') !!}
    <p>{{ $tXN->source_ip }}</p>
</div>

<!-- Latest Ip Field -->
<div class="form-group">
    {!! Form::label('latest_ip', 'Latest Ip:') !!}
    <p>{{ $tXN->latest_ip }}</p>
</div>

<!-- Prev Iso Id Field -->
<div class="form-group">
    {!! Form::label('prev_iso_id', 'Prev Iso Id:') !!}
    <p>{{ $tXN->prev_iso_id }}</p>
</div>

<!-- Company Id Field -->
<div class="form-group">
    {!! Form::label('company_id', 'Company Id:') !!}
    <p>{{ $tXN->company_id }}</p>
</div>

<!-- Need Sync Field -->
<div class="form-group">
    {!! Form::label('need_sync', 'Need Sync:') !!}
    <p>{{ $tXN->need_sync }}</p>
</div>

<!-- Synced Field -->
<div class="form-group">
    {!! Form::label('synced', 'Synced:') !!}
    <p>{{ $tXN->synced }}</p>
</div>

<!-- Iso Source Field -->
<div class="form-group">
    {!! Form::label('iso_source', 'Iso Source:') !!}
    <p>{{ $tXN->iso_source }}</p>
</div>

<!-- Iso Type Field -->
<div class="form-group">
    {!! Form::label('iso_type', 'Iso Type:') !!}
    <p>{{ $tXN->iso_type }}</p>
</div>

<!-- Request Type Field -->
<div class="form-group">
    {!! Form::label('request_type', 'Request Type:') !!}
    <p>{{ $tXN->request_type }}</p>
</div>

<!-- Iso Status Field -->
<div class="form-group">
    {!! Form::label('iso_status', 'Iso Status:') !!}
    <p>{{ $tXN->iso_status }}</p>
</div>

<!-- Iso Version Field -->
<div class="form-group">
    {!! Form::label('iso_version', 'Iso Version:') !!}
    <p>{{ $tXN->iso_version }}</p>
</div>

<!-- Req Mti Field -->
<div class="form-group">
    {!! Form::label('req_mti', 'Req Mti:') !!}
    <p>{{ $tXN->req_mti }}</p>
</div>

<!-- Req Field1 Field -->
<div class="form-group">
    {!! Form::label('req_field1', 'Req Field1:') !!}
    <p>{{ $tXN->req_field1 }}</p>
</div>

<!-- Req Field2 Field -->
<div class="form-group">
    {!! Form::label('req_field2', 'Req Field2:') !!}
    <p>{{ $tXN->req_field2 }}</p>
</div>

<!-- Req Field3 Field -->
<div class="form-group">
    {!! Form::label('req_field3', 'Req Field3:') !!}
    <p>{{ $tXN->req_field3 }}</p>
</div>

<!-- Req Field4 Field -->
<div class="form-group">
    {!! Form::label('req_field4', 'Req Field4:') !!}
    <p>{{ $tXN->req_field4 }}</p>
</div>

<!-- Req Field5 Field -->
<div class="form-group">
    {!! Form::label('req_field5', 'Req Field5:') !!}
    <p>{{ $tXN->req_field5 }}</p>
</div>

<!-- Req Field6 Field -->
<div class="form-group">
    {!! Form::label('req_field6', 'Req Field6:') !!}
    <p>{{ $tXN->req_field6 }}</p>
</div>

<!-- Req Field7 Field -->
<div class="form-group">
    {!! Form::label('req_field7', 'Req Field7:') !!}
    <p>{{ $tXN->req_field7 }}</p>
</div>

<!-- Req Field8 Field -->
<div class="form-group">
    {!! Form::label('req_field8', 'Req Field8:') !!}
    <p>{{ $tXN->req_field8 }}</p>
</div>

<!-- Req Field9 Field -->
<div class="form-group">
    {!! Form::label('req_field9', 'Req Field9:') !!}
    <p>{{ $tXN->req_field9 }}</p>
</div>

<!-- Req Field10 Field -->
<div class="form-group">
    {!! Form::label('req_field10', 'Req Field10:') !!}
    <p>{{ $tXN->req_field10 }}</p>
</div>

<!-- Req Field11 Field -->
<div class="form-group">
    {!! Form::label('req_field11', 'Req Field11:') !!}
    <p>{{ $tXN->req_field11 }}</p>
</div>

<!-- Req Field12 Field -->
<div class="form-group">
    {!! Form::label('req_field12', 'Req Field12:') !!}
    <p>{{ $tXN->req_field12 }}</p>
</div>

<!-- Req Field13 Field -->
<div class="form-group">
    {!! Form::label('req_field13', 'Req Field13:') !!}
    <p>{{ $tXN->req_field13 }}</p>
</div>

<!-- Req Field14 Field -->
<div class="form-group">
    {!! Form::label('req_field14', 'Req Field14:') !!}
    <p>{{ $tXN->req_field14 }}</p>
</div>

<!-- Req Field15 Field -->
<div class="form-group">
    {!! Form::label('req_field15', 'Req Field15:') !!}
    <p>{{ $tXN->req_field15 }}</p>
</div>

<!-- Req Field16 Field -->
<div class="form-group">
    {!! Form::label('req_field16', 'Req Field16:') !!}
    <p>{{ $tXN->req_field16 }}</p>
</div>

<!-- Req Field17 Field -->
<div class="form-group">
    {!! Form::label('req_field17', 'Req Field17:') !!}
    <p>{{ $tXN->req_field17 }}</p>
</div>

<!-- Req Field18 Field -->
<div class="form-group">
    {!! Form::label('req_field18', 'Req Field18:') !!}
    <p>{{ $tXN->req_field18 }}</p>
</div>

<!-- Req Field19 Field -->
<div class="form-group">
    {!! Form::label('req_field19', 'Req Field19:') !!}
    <p>{{ $tXN->req_field19 }}</p>
</div>

<!-- Req Field20 Field -->
<div class="form-group">
    {!! Form::label('req_field20', 'Req Field20:') !!}
    <p>{{ $tXN->req_field20 }}</p>
</div>

<!-- Req Field21 Field -->
<div class="form-group">
    {!! Form::label('req_field21', 'Req Field21:') !!}
    <p>{{ $tXN->req_field21 }}</p>
</div>

<!-- Req Field22 Field -->
<div class="form-group">
    {!! Form::label('req_field22', 'Req Field22:') !!}
    <p>{{ $tXN->req_field22 }}</p>
</div>

<!-- Req Field23 Field -->
<div class="form-group">
    {!! Form::label('req_field23', 'Req Field23:') !!}
    <p>{{ $tXN->req_field23 }}</p>
</div>

<!-- Req Field24 Field -->
<div class="form-group">
    {!! Form::label('req_field24', 'Req Field24:') !!}
    <p>{{ $tXN->req_field24 }}</p>
</div>

<!-- Req Field25 Field -->
<div class="form-group">
    {!! Form::label('req_field25', 'Req Field25:') !!}
    <p>{{ $tXN->req_field25 }}</p>
</div>

<!-- Req Field26 Field -->
<div class="form-group">
    {!! Form::label('req_field26', 'Req Field26:') !!}
    <p>{{ $tXN->req_field26 }}</p>
</div>

<!-- Req Field27 Field -->
<div class="form-group">
    {!! Form::label('req_field27', 'Req Field27:') !!}
    <p>{{ $tXN->req_field27 }}</p>
</div>

<!-- Req Field28 Field -->
<div class="form-group">
    {!! Form::label('req_field28', 'Req Field28:') !!}
    <p>{{ $tXN->req_field28 }}</p>
</div>

<!-- Req Field29 Field -->
<div class="form-group">
    {!! Form::label('req_field29', 'Req Field29:') !!}
    <p>{{ $tXN->req_field29 }}</p>
</div>

<!-- Req Field30 Field -->
<div class="form-group">
    {!! Form::label('req_field30', 'Req Field30:') !!}
    <p>{{ $tXN->req_field30 }}</p>
</div>

<!-- Req Field31 Field -->
<div class="form-group">
    {!! Form::label('req_field31', 'Req Field31:') !!}
    <p>{{ $tXN->req_field31 }}</p>
</div>

<!-- Req Field32 Field -->
<div class="form-group">
    {!! Form::label('req_field32', 'Req Field32:') !!}
    <p>{{ $tXN->req_field32 }}</p>
</div>

<!-- Req Field33 Field -->
<div class="form-group">
    {!! Form::label('req_field33', 'Req Field33:') !!}
    <p>{{ $tXN->req_field33 }}</p>
</div>

<!-- Req Field34 Field -->
<div class="form-group">
    {!! Form::label('req_field34', 'Req Field34:') !!}
    <p>{{ $tXN->req_field34 }}</p>
</div>

<!-- Req Field35 Field -->
<div class="form-group">
    {!! Form::label('req_field35', 'Req Field35:') !!}
    <p>{{ $tXN->req_field35 }}</p>
</div>

<!-- Req Field36 Field -->
<div class="form-group">
    {!! Form::label('req_field36', 'Req Field36:') !!}
    <p>{{ $tXN->req_field36 }}</p>
</div>

<!-- Req Field37 Field -->
<div class="form-group">
    {!! Form::label('req_field37', 'Req Field37:') !!}
    <p>{{ $tXN->req_field37 }}</p>
</div>

<!-- Req Field38 Field -->
<div class="form-group">
    {!! Form::label('req_field38', 'Req Field38:') !!}
    <p>{{ $tXN->req_field38 }}</p>
</div>

<!-- Req Field39 Field -->
<div class="form-group">
    {!! Form::label('req_field39', 'Req Field39:') !!}
    <p>{{ $tXN->req_field39 }}</p>
</div>

<!-- Req Field40 Field -->
<div class="form-group">
    {!! Form::label('req_field40', 'Req Field40:') !!}
    <p>{{ $tXN->req_field40 }}</p>
</div>

<!-- Req Field41 Field -->
<div class="form-group">
    {!! Form::label('req_field41', 'Req Field41:') !!}
    <p>{{ $tXN->req_field41 }}</p>
</div>

<!-- Req Field42 Field -->
<div class="form-group">
    {!! Form::label('req_field42', 'Req Field42:') !!}
    <p>{{ $tXN->req_field42 }}</p>
</div>

<!-- Req Field43 Field -->
<div class="form-group">
    {!! Form::label('req_field43', 'Req Field43:') !!}
    <p>{{ $tXN->req_field43 }}</p>
</div>

<!-- Req Field44 Field -->
<div class="form-group">
    {!! Form::label('req_field44', 'Req Field44:') !!}
    <p>{{ $tXN->req_field44 }}</p>
</div>

<!-- Req Field45 Field -->
<div class="form-group">
    {!! Form::label('req_field45', 'Req Field45:') !!}
    <p>{{ $tXN->req_field45 }}</p>
</div>

<!-- Req Field46 Field -->
<div class="form-group">
    {!! Form::label('req_field46', 'Req Field46:') !!}
    <p>{{ $tXN->req_field46 }}</p>
</div>

<!-- Req Field47 Field -->
<div class="form-group">
    {!! Form::label('req_field47', 'Req Field47:') !!}
    <p>{{ $tXN->req_field47 }}</p>
</div>

<!-- Req Field48 Field -->
<div class="form-group">
    {!! Form::label('req_field48', 'Req Field48:') !!}
    <p>{{ $tXN->req_field48 }}</p>
</div>

<!-- Req Field49 Field -->
<div class="form-group">
    {!! Form::label('req_field49', 'Req Field49:') !!}
    <p>{{ $tXN->req_field49 }}</p>
</div>

<!-- Req Field50 Field -->
<div class="form-group">
    {!! Form::label('req_field50', 'Req Field50:') !!}
    <p>{{ $tXN->req_field50 }}</p>
</div>

<!-- Req Field51 Field -->
<div class="form-group">
    {!! Form::label('req_field51', 'Req Field51:') !!}
    <p>{{ $tXN->req_field51 }}</p>
</div>

<!-- Req Field52 Field -->
<div class="form-group">
    {!! Form::label('req_field52', 'Req Field52:') !!}
    <p>{{ $tXN->req_field52 }}</p>
</div>

<!-- Req Field53 Field -->
<div class="form-group">
    {!! Form::label('req_field53', 'Req Field53:') !!}
    <p>{{ $tXN->req_field53 }}</p>
</div>

<!-- Req Field54 Field -->
<div class="form-group">
    {!! Form::label('req_field54', 'Req Field54:') !!}
    <p>{{ $tXN->req_field54 }}</p>
</div>

<!-- Req Field55 Field -->
<div class="form-group">
    {!! Form::label('req_field55', 'Req Field55:') !!}
    <p>{{ $tXN->req_field55 }}</p>
</div>

<!-- Req Field56 Field -->
<div class="form-group">
    {!! Form::label('req_field56', 'Req Field56:') !!}
    <p>{{ $tXN->req_field56 }}</p>
</div>

<!-- Req Field57 Field -->
<div class="form-group">
    {!! Form::label('req_field57', 'Req Field57:') !!}
    <p>{{ $tXN->req_field57 }}</p>
</div>

<!-- Req Field58 Field -->
<div class="form-group">
    {!! Form::label('req_field58', 'Req Field58:') !!}
    <p>{{ $tXN->req_field58 }}</p>
</div>

<!-- Req Field59 Field -->
<div class="form-group">
    {!! Form::label('req_field59', 'Req Field59:') !!}
    <p>{{ $tXN->req_field59 }}</p>
</div>

<!-- Req Field60 Field -->
<div class="form-group">
    {!! Form::label('req_field60', 'Req Field60:') !!}
    <p>{{ $tXN->req_field60 }}</p>
</div>

<!-- Req Field61 Field -->
<div class="form-group">
    {!! Form::label('req_field61', 'Req Field61:') !!}
    <p>{{ $tXN->req_field61 }}</p>
</div>

<!-- Req Field62 Field -->
<div class="form-group">
    {!! Form::label('req_field62', 'Req Field62:') !!}
    <p>{{ $tXN->req_field62 }}</p>
</div>

<!-- Req Field63 Field -->
<div class="form-group">
    {!! Form::label('req_field63', 'Req Field63:') !!}
    <p>{{ $tXN->req_field63 }}</p>
</div>

<!-- Req Field64 Field -->
<div class="form-group">
    {!! Form::label('req_field64', 'Req Field64:') !!}
    <p>{{ $tXN->req_field64 }}</p>
</div>

<!-- Req Field65 Field -->
<div class="form-group">
    {!! Form::label('req_field65', 'Req Field65:') !!}
    <p>{{ $tXN->req_field65 }}</p>
</div>

<!-- Req Field66 Field -->
<div class="form-group">
    {!! Form::label('req_field66', 'Req Field66:') !!}
    <p>{{ $tXN->req_field66 }}</p>
</div>

<!-- Req Field67 Field -->
<div class="form-group">
    {!! Form::label('req_field67', 'Req Field67:') !!}
    <p>{{ $tXN->req_field67 }}</p>
</div>

<!-- Req Field68 Field -->
<div class="form-group">
    {!! Form::label('req_field68', 'Req Field68:') !!}
    <p>{{ $tXN->req_field68 }}</p>
</div>

<!-- Req Field69 Field -->
<div class="form-group">
    {!! Form::label('req_field69', 'Req Field69:') !!}
    <p>{{ $tXN->req_field69 }}</p>
</div>

<!-- Req Field70 Field -->
<div class="form-group">
    {!! Form::label('req_field70', 'Req Field70:') !!}
    <p>{{ $tXN->req_field70 }}</p>
</div>

<!-- Req Field71 Field -->
<div class="form-group">
    {!! Form::label('req_field71', 'Req Field71:') !!}
    <p>{{ $tXN->req_field71 }}</p>
</div>

<!-- Req Field72 Field -->
<div class="form-group">
    {!! Form::label('req_field72', 'Req Field72:') !!}
    <p>{{ $tXN->req_field72 }}</p>
</div>

<!-- Req Field73 Field -->
<div class="form-group">
    {!! Form::label('req_field73', 'Req Field73:') !!}
    <p>{{ $tXN->req_field73 }}</p>
</div>

<!-- Req Field74 Field -->
<div class="form-group">
    {!! Form::label('req_field74', 'Req Field74:') !!}
    <p>{{ $tXN->req_field74 }}</p>
</div>

<!-- Req Field75 Field -->
<div class="form-group">
    {!! Form::label('req_field75', 'Req Field75:') !!}
    <p>{{ $tXN->req_field75 }}</p>
</div>

<!-- Req Field76 Field -->
<div class="form-group">
    {!! Form::label('req_field76', 'Req Field76:') !!}
    <p>{{ $tXN->req_field76 }}</p>
</div>

<!-- Req Field77 Field -->
<div class="form-group">
    {!! Form::label('req_field77', 'Req Field77:') !!}
    <p>{{ $tXN->req_field77 }}</p>
</div>

<!-- Req Field78 Field -->
<div class="form-group">
    {!! Form::label('req_field78', 'Req Field78:') !!}
    <p>{{ $tXN->req_field78 }}</p>
</div>

<!-- Req Field79 Field -->
<div class="form-group">
    {!! Form::label('req_field79', 'Req Field79:') !!}
    <p>{{ $tXN->req_field79 }}</p>
</div>

<!-- Req Field80 Field -->
<div class="form-group">
    {!! Form::label('req_field80', 'Req Field80:') !!}
    <p>{{ $tXN->req_field80 }}</p>
</div>

<!-- Req Field81 Field -->
<div class="form-group">
    {!! Form::label('req_field81', 'Req Field81:') !!}
    <p>{{ $tXN->req_field81 }}</p>
</div>

<!-- Req Field82 Field -->
<div class="form-group">
    {!! Form::label('req_field82', 'Req Field82:') !!}
    <p>{{ $tXN->req_field82 }}</p>
</div>

<!-- Req Field83 Field -->
<div class="form-group">
    {!! Form::label('req_field83', 'Req Field83:') !!}
    <p>{{ $tXN->req_field83 }}</p>
</div>

<!-- Req Field84 Field -->
<div class="form-group">
    {!! Form::label('req_field84', 'Req Field84:') !!}
    <p>{{ $tXN->req_field84 }}</p>
</div>

<!-- Req Field85 Field -->
<div class="form-group">
    {!! Form::label('req_field85', 'Req Field85:') !!}
    <p>{{ $tXN->req_field85 }}</p>
</div>

<!-- Req Field86 Field -->
<div class="form-group">
    {!! Form::label('req_field86', 'Req Field86:') !!}
    <p>{{ $tXN->req_field86 }}</p>
</div>

<!-- Req Field87 Field -->
<div class="form-group">
    {!! Form::label('req_field87', 'Req Field87:') !!}
    <p>{{ $tXN->req_field87 }}</p>
</div>

<!-- Req Field88 Field -->
<div class="form-group">
    {!! Form::label('req_field88', 'Req Field88:') !!}
    <p>{{ $tXN->req_field88 }}</p>
</div>

<!-- Req Field89 Field -->
<div class="form-group">
    {!! Form::label('req_field89', 'Req Field89:') !!}
    <p>{{ $tXN->req_field89 }}</p>
</div>

<!-- Req Field90 Field -->
<div class="form-group">
    {!! Form::label('req_field90', 'Req Field90:') !!}
    <p>{{ $tXN->req_field90 }}</p>
</div>

<!-- Req Field91 Field -->
<div class="form-group">
    {!! Form::label('req_field91', 'Req Field91:') !!}
    <p>{{ $tXN->req_field91 }}</p>
</div>

<!-- Req Field92 Field -->
<div class="form-group">
    {!! Form::label('req_field92', 'Req Field92:') !!}
    <p>{{ $tXN->req_field92 }}</p>
</div>

<!-- Req Field93 Field -->
<div class="form-group">
    {!! Form::label('req_field93', 'Req Field93:') !!}
    <p>{{ $tXN->req_field93 }}</p>
</div>

<!-- Req Field94 Field -->
<div class="form-group">
    {!! Form::label('req_field94', 'Req Field94:') !!}
    <p>{{ $tXN->req_field94 }}</p>
</div>

<!-- Req Field95 Field -->
<div class="form-group">
    {!! Form::label('req_field95', 'Req Field95:') !!}
    <p>{{ $tXN->req_field95 }}</p>
</div>

<!-- Req Field96 Field -->
<div class="form-group">
    {!! Form::label('req_field96', 'Req Field96:') !!}
    <p>{{ $tXN->req_field96 }}</p>
</div>

<!-- Req Field97 Field -->
<div class="form-group">
    {!! Form::label('req_field97', 'Req Field97:') !!}
    <p>{{ $tXN->req_field97 }}</p>
</div>

<!-- Req Field98 Field -->
<div class="form-group">
    {!! Form::label('req_field98', 'Req Field98:') !!}
    <p>{{ $tXN->req_field98 }}</p>
</div>

<!-- Req Field99 Field -->
<div class="form-group">
    {!! Form::label('req_field99', 'Req Field99:') !!}
    <p>{{ $tXN->req_field99 }}</p>
</div>

<!-- Req Field100 Field -->
<div class="form-group">
    {!! Form::label('req_field100', 'Req Field100:') !!}
    <p>{{ $tXN->req_field100 }}</p>
</div>

<!-- Req Field101 Field -->
<div class="form-group">
    {!! Form::label('req_field101', 'Req Field101:') !!}
    <p>{{ $tXN->req_field101 }}</p>
</div>

<!-- Req Field102 Field -->
<div class="form-group">
    {!! Form::label('req_field102', 'Req Field102:') !!}
    <p>{{ $tXN->req_field102 }}</p>
</div>

<!-- Req Field103 Field -->
<div class="form-group">
    {!! Form::label('req_field103', 'Req Field103:') !!}
    <p>{{ $tXN->req_field103 }}</p>
</div>

<!-- Req Field104 Field -->
<div class="form-group">
    {!! Form::label('req_field104', 'Req Field104:') !!}
    <p>{{ $tXN->req_field104 }}</p>
</div>

<!-- Req Field105 Field -->
<div class="form-group">
    {!! Form::label('req_field105', 'Req Field105:') !!}
    <p>{{ $tXN->req_field105 }}</p>
</div>

<!-- Req Field106 Field -->
<div class="form-group">
    {!! Form::label('req_field106', 'Req Field106:') !!}
    <p>{{ $tXN->req_field106 }}</p>
</div>

<!-- Req Field107 Field -->
<div class="form-group">
    {!! Form::label('req_field107', 'Req Field107:') !!}
    <p>{{ $tXN->req_field107 }}</p>
</div>

<!-- Req Field108 Field -->
<div class="form-group">
    {!! Form::label('req_field108', 'Req Field108:') !!}
    <p>{{ $tXN->req_field108 }}</p>
</div>

<!-- Req Field109 Field -->
<div class="form-group">
    {!! Form::label('req_field109', 'Req Field109:') !!}
    <p>{{ $tXN->req_field109 }}</p>
</div>

<!-- Req Field110 Field -->
<div class="form-group">
    {!! Form::label('req_field110', 'Req Field110:') !!}
    <p>{{ $tXN->req_field110 }}</p>
</div>

<!-- Req Field111 Field -->
<div class="form-group">
    {!! Form::label('req_field111', 'Req Field111:') !!}
    <p>{{ $tXN->req_field111 }}</p>
</div>

<!-- Req Field112 Field -->
<div class="form-group">
    {!! Form::label('req_field112', 'Req Field112:') !!}
    <p>{{ $tXN->req_field112 }}</p>
</div>

<!-- Req Field113 Field -->
<div class="form-group">
    {!! Form::label('req_field113', 'Req Field113:') !!}
    <p>{{ $tXN->req_field113 }}</p>
</div>

<!-- Req Field114 Field -->
<div class="form-group">
    {!! Form::label('req_field114', 'Req Field114:') !!}
    <p>{{ $tXN->req_field114 }}</p>
</div>

<!-- Req Field115 Field -->
<div class="form-group">
    {!! Form::label('req_field115', 'Req Field115:') !!}
    <p>{{ $tXN->req_field115 }}</p>
</div>

<!-- Req Field116 Field -->
<div class="form-group">
    {!! Form::label('req_field116', 'Req Field116:') !!}
    <p>{{ $tXN->req_field116 }}</p>
</div>

<!-- Req Field117 Field -->
<div class="form-group">
    {!! Form::label('req_field117', 'Req Field117:') !!}
    <p>{{ $tXN->req_field117 }}</p>
</div>

<!-- Req Field118 Field -->
<div class="form-group">
    {!! Form::label('req_field118', 'Req Field118:') !!}
    <p>{{ $tXN->req_field118 }}</p>
</div>

<!-- Req Field119 Field -->
<div class="form-group">
    {!! Form::label('req_field119', 'Req Field119:') !!}
    <p>{{ $tXN->req_field119 }}</p>
</div>

<!-- Req Field120 Field -->
<div class="form-group">
    {!! Form::label('req_field120', 'Req Field120:') !!}
    <p>{{ $tXN->req_field120 }}</p>
</div>

<!-- Req Field121 Field -->
<div class="form-group">
    {!! Form::label('req_field121', 'Req Field121:') !!}
    <p>{{ $tXN->req_field121 }}</p>
</div>

<!-- Req Field122 Field -->
<div class="form-group">
    {!! Form::label('req_field122', 'Req Field122:') !!}
    <p>{{ $tXN->req_field122 }}</p>
</div>

<!-- Req Field123 Field -->
<div class="form-group">
    {!! Form::label('req_field123', 'Req Field123:') !!}
    <p>{{ $tXN->req_field123 }}</p>
</div>

<!-- Req Field124 Field -->
<div class="form-group">
    {!! Form::label('req_field124', 'Req Field124:') !!}
    <p>{{ $tXN->req_field124 }}</p>
</div>

<!-- Req Field125 Field -->
<div class="form-group">
    {!! Form::label('req_field125', 'Req Field125:') !!}
    <p>{{ $tXN->req_field125 }}</p>
</div>

<!-- Req Field126 Field -->
<div class="form-group">
    {!! Form::label('req_field126', 'Req Field126:') !!}
    <p>{{ $tXN->req_field126 }}</p>
</div>

<!-- Req Field127 Field -->
<div class="form-group">
    {!! Form::label('req_field127', 'Req Field127:') !!}
    <p>{{ $tXN->req_field127 }}</p>
</div>

<!-- Req Field128 Field -->
<div class="form-group">
    {!! Form::label('req_field128', 'Req Field128:') !!}
    <p>{{ $tXN->req_field128 }}</p>
</div>

<!-- Res Mti Field -->
<div class="form-group">
    {!! Form::label('res_mti', 'Res Mti:') !!}
    <p>{{ $tXN->res_mti }}</p>
</div>

<!-- Res Field1 Field -->
<div class="form-group">
    {!! Form::label('res_field1', 'Res Field1:') !!}
    <p>{{ $tXN->res_field1 }}</p>
</div>

<!-- Res Field2 Field -->
<div class="form-group">
    {!! Form::label('res_field2', 'Res Field2:') !!}
    <p>{{ $tXN->res_field2 }}</p>
</div>

<!-- Res Field3 Field -->
<div class="form-group">
    {!! Form::label('res_field3', 'Res Field3:') !!}
    <p>{{ $tXN->res_field3 }}</p>
</div>

<!-- Res Field4 Field -->
<div class="form-group">
    {!! Form::label('res_field4', 'Res Field4:') !!}
    <p>{{ $tXN->res_field4 }}</p>
</div>

<!-- Res Field5 Field -->
<div class="form-group">
    {!! Form::label('res_field5', 'Res Field5:') !!}
    <p>{{ $tXN->res_field5 }}</p>
</div>

<!-- Res Field6 Field -->
<div class="form-group">
    {!! Form::label('res_field6', 'Res Field6:') !!}
    <p>{{ $tXN->res_field6 }}</p>
</div>

<!-- Res Field7 Field -->
<div class="form-group">
    {!! Form::label('res_field7', 'Res Field7:') !!}
    <p>{{ $tXN->res_field7 }}</p>
</div>

<!-- Res Field8 Field -->
<div class="form-group">
    {!! Form::label('res_field8', 'Res Field8:') !!}
    <p>{{ $tXN->res_field8 }}</p>
</div>

<!-- Res Field9 Field -->
<div class="form-group">
    {!! Form::label('res_field9', 'Res Field9:') !!}
    <p>{{ $tXN->res_field9 }}</p>
</div>

<!-- Res Field10 Field -->
<div class="form-group">
    {!! Form::label('res_field10', 'Res Field10:') !!}
    <p>{{ $tXN->res_field10 }}</p>
</div>

<!-- Res Field11 Field -->
<div class="form-group">
    {!! Form::label('res_field11', 'Res Field11:') !!}
    <p>{{ $tXN->res_field11 }}</p>
</div>

<!-- Res Field12 Field -->
<div class="form-group">
    {!! Form::label('res_field12', 'Res Field12:') !!}
    <p>{{ $tXN->res_field12 }}</p>
</div>

<!-- Res Field13 Field -->
<div class="form-group">
    {!! Form::label('res_field13', 'Res Field13:') !!}
    <p>{{ $tXN->res_field13 }}</p>
</div>

<!-- Res Field14 Field -->
<div class="form-group">
    {!! Form::label('res_field14', 'Res Field14:') !!}
    <p>{{ $tXN->res_field14 }}</p>
</div>

<!-- Res Field15 Field -->
<div class="form-group">
    {!! Form::label('res_field15', 'Res Field15:') !!}
    <p>{{ $tXN->res_field15 }}</p>
</div>

<!-- Res Field16 Field -->
<div class="form-group">
    {!! Form::label('res_field16', 'Res Field16:') !!}
    <p>{{ $tXN->res_field16 }}</p>
</div>

<!-- Res Field17 Field -->
<div class="form-group">
    {!! Form::label('res_field17', 'Res Field17:') !!}
    <p>{{ $tXN->res_field17 }}</p>
</div>

<!-- Res Field18 Field -->
<div class="form-group">
    {!! Form::label('res_field18', 'Res Field18:') !!}
    <p>{{ $tXN->res_field18 }}</p>
</div>

<!-- Res Field19 Field -->
<div class="form-group">
    {!! Form::label('res_field19', 'Res Field19:') !!}
    <p>{{ $tXN->res_field19 }}</p>
</div>

<!-- Res Field20 Field -->
<div class="form-group">
    {!! Form::label('res_field20', 'Res Field20:') !!}
    <p>{{ $tXN->res_field20 }}</p>
</div>

<!-- Res Field21 Field -->
<div class="form-group">
    {!! Form::label('res_field21', 'Res Field21:') !!}
    <p>{{ $tXN->res_field21 }}</p>
</div>

<!-- Res Field22 Field -->
<div class="form-group">
    {!! Form::label('res_field22', 'Res Field22:') !!}
    <p>{{ $tXN->res_field22 }}</p>
</div>

<!-- Res Field23 Field -->
<div class="form-group">
    {!! Form::label('res_field23', 'Res Field23:') !!}
    <p>{{ $tXN->res_field23 }}</p>
</div>

<!-- Res Field24 Field -->
<div class="form-group">
    {!! Form::label('res_field24', 'Res Field24:') !!}
    <p>{{ $tXN->res_field24 }}</p>
</div>

<!-- Res Field25 Field -->
<div class="form-group">
    {!! Form::label('res_field25', 'Res Field25:') !!}
    <p>{{ $tXN->res_field25 }}</p>
</div>

<!-- Res Field26 Field -->
<div class="form-group">
    {!! Form::label('res_field26', 'Res Field26:') !!}
    <p>{{ $tXN->res_field26 }}</p>
</div>

<!-- Res Field27 Field -->
<div class="form-group">
    {!! Form::label('res_field27', 'Res Field27:') !!}
    <p>{{ $tXN->res_field27 }}</p>
</div>

<!-- Res Field28 Field -->
<div class="form-group">
    {!! Form::label('res_field28', 'Res Field28:') !!}
    <p>{{ $tXN->res_field28 }}</p>
</div>

<!-- Res Field29 Field -->
<div class="form-group">
    {!! Form::label('res_field29', 'Res Field29:') !!}
    <p>{{ $tXN->res_field29 }}</p>
</div>

<!-- Res Field30 Field -->
<div class="form-group">
    {!! Form::label('res_field30', 'Res Field30:') !!}
    <p>{{ $tXN->res_field30 }}</p>
</div>

<!-- Res Field31 Field -->
<div class="form-group">
    {!! Form::label('res_field31', 'Res Field31:') !!}
    <p>{{ $tXN->res_field31 }}</p>
</div>

<!-- Res Field32 Field -->
<div class="form-group">
    {!! Form::label('res_field32', 'Res Field32:') !!}
    <p>{{ $tXN->res_field32 }}</p>
</div>

<!-- Res Field33 Field -->
<div class="form-group">
    {!! Form::label('res_field33', 'Res Field33:') !!}
    <p>{{ $tXN->res_field33 }}</p>
</div>

<!-- Res Field34 Field -->
<div class="form-group">
    {!! Form::label('res_field34', 'Res Field34:') !!}
    <p>{{ $tXN->res_field34 }}</p>
</div>

<!-- Res Field35 Field -->
<div class="form-group">
    {!! Form::label('res_field35', 'Res Field35:') !!}
    <p>{{ $tXN->res_field35 }}</p>
</div>

<!-- Res Field36 Field -->
<div class="form-group">
    {!! Form::label('res_field36', 'Res Field36:') !!}
    <p>{{ $tXN->res_field36 }}</p>
</div>

<!-- Res Field37 Field -->
<div class="form-group">
    {!! Form::label('res_field37', 'Res Field37:') !!}
    <p>{{ $tXN->res_field37 }}</p>
</div>

<!-- Res Field38 Field -->
<div class="form-group">
    {!! Form::label('res_field38', 'Res Field38:') !!}
    <p>{{ $tXN->res_field38 }}</p>
</div>

<!-- Res Field39 Field -->
<div class="form-group">
    {!! Form::label('res_field39', 'Res Field39:') !!}
    <p>{{ $tXN->res_field39 }}</p>
</div>

<!-- Res Field40 Field -->
<div class="form-group">
    {!! Form::label('res_field40', 'Res Field40:') !!}
    <p>{{ $tXN->res_field40 }}</p>
</div>

<!-- Res Field41 Field -->
<div class="form-group">
    {!! Form::label('res_field41', 'Res Field41:') !!}
    <p>{{ $tXN->res_field41 }}</p>
</div>

<!-- Res Field42 Field -->
<div class="form-group">
    {!! Form::label('res_field42', 'Res Field42:') !!}
    <p>{{ $tXN->res_field42 }}</p>
</div>

<!-- Res Field43 Field -->
<div class="form-group">
    {!! Form::label('res_field43', 'Res Field43:') !!}
    <p>{{ $tXN->res_field43 }}</p>
</div>

<!-- Res Field44 Field -->
<div class="form-group">
    {!! Form::label('res_field44', 'Res Field44:') !!}
    <p>{{ $tXN->res_field44 }}</p>
</div>

<!-- Res Field45 Field -->
<div class="form-group">
    {!! Form::label('res_field45', 'Res Field45:') !!}
    <p>{{ $tXN->res_field45 }}</p>
</div>

<!-- Res Field46 Field -->
<div class="form-group">
    {!! Form::label('res_field46', 'Res Field46:') !!}
    <p>{{ $tXN->res_field46 }}</p>
</div>

<!-- Res Field47 Field -->
<div class="form-group">
    {!! Form::label('res_field47', 'Res Field47:') !!}
    <p>{{ $tXN->res_field47 }}</p>
</div>

<!-- Res Field48 Field -->
<div class="form-group">
    {!! Form::label('res_field48', 'Res Field48:') !!}
    <p>{{ $tXN->res_field48 }}</p>
</div>

<!-- Res Field49 Field -->
<div class="form-group">
    {!! Form::label('res_field49', 'Res Field49:') !!}
    <p>{{ $tXN->res_field49 }}</p>
</div>

<!-- Res Field50 Field -->
<div class="form-group">
    {!! Form::label('res_field50', 'Res Field50:') !!}
    <p>{{ $tXN->res_field50 }}</p>
</div>

<!-- Res Field51 Field -->
<div class="form-group">
    {!! Form::label('res_field51', 'Res Field51:') !!}
    <p>{{ $tXN->res_field51 }}</p>
</div>

<!-- Res Field52 Field -->
<div class="form-group">
    {!! Form::label('res_field52', 'Res Field52:') !!}
    <p>{{ $tXN->res_field52 }}</p>
</div>

<!-- Res Field53 Field -->
<div class="form-group">
    {!! Form::label('res_field53', 'Res Field53:') !!}
    <p>{{ $tXN->res_field53 }}</p>
</div>

<!-- Res Field54 Field -->
<div class="form-group">
    {!! Form::label('res_field54', 'Res Field54:') !!}
    <p>{{ $tXN->res_field54 }}</p>
</div>

<!-- Res Field55 Field -->
<div class="form-group">
    {!! Form::label('res_field55', 'Res Field55:') !!}
    <p>{{ $tXN->res_field55 }}</p>
</div>

<!-- Res Field56 Field -->
<div class="form-group">
    {!! Form::label('res_field56', 'Res Field56:') !!}
    <p>{{ $tXN->res_field56 }}</p>
</div>

<!-- Res Field57 Field -->
<div class="form-group">
    {!! Form::label('res_field57', 'Res Field57:') !!}
    <p>{{ $tXN->res_field57 }}</p>
</div>

<!-- Res Field58 Field -->
<div class="form-group">
    {!! Form::label('res_field58', 'Res Field58:') !!}
    <p>{{ $tXN->res_field58 }}</p>
</div>

<!-- Res Field59 Field -->
<div class="form-group">
    {!! Form::label('res_field59', 'Res Field59:') !!}
    <p>{{ $tXN->res_field59 }}</p>
</div>

<!-- Res Field60 Field -->
<div class="form-group">
    {!! Form::label('res_field60', 'Res Field60:') !!}
    <p>{{ $tXN->res_field60 }}</p>
</div>

<!-- Res Field61 Field -->
<div class="form-group">
    {!! Form::label('res_field61', 'Res Field61:') !!}
    <p>{{ $tXN->res_field61 }}</p>
</div>

<!-- Res Field62 Field -->
<div class="form-group">
    {!! Form::label('res_field62', 'Res Field62:') !!}
    <p>{{ $tXN->res_field62 }}</p>
</div>

<!-- Res Field63 Field -->
<div class="form-group">
    {!! Form::label('res_field63', 'Res Field63:') !!}
    <p>{{ $tXN->res_field63 }}</p>
</div>

<!-- Res Field64 Field -->
<div class="form-group">
    {!! Form::label('res_field64', 'Res Field64:') !!}
    <p>{{ $tXN->res_field64 }}</p>
</div>

<!-- Res Field65 Field -->
<div class="form-group">
    {!! Form::label('res_field65', 'Res Field65:') !!}
    <p>{{ $tXN->res_field65 }}</p>
</div>

<!-- Res Field66 Field -->
<div class="form-group">
    {!! Form::label('res_field66', 'Res Field66:') !!}
    <p>{{ $tXN->res_field66 }}</p>
</div>

<!-- Res Field67 Field -->
<div class="form-group">
    {!! Form::label('res_field67', 'Res Field67:') !!}
    <p>{{ $tXN->res_field67 }}</p>
</div>

<!-- Res Field68 Field -->
<div class="form-group">
    {!! Form::label('res_field68', 'Res Field68:') !!}
    <p>{{ $tXN->res_field68 }}</p>
</div>

<!-- Res Field69 Field -->
<div class="form-group">
    {!! Form::label('res_field69', 'Res Field69:') !!}
    <p>{{ $tXN->res_field69 }}</p>
</div>

<!-- Res Field70 Field -->
<div class="form-group">
    {!! Form::label('res_field70', 'Res Field70:') !!}
    <p>{{ $tXN->res_field70 }}</p>
</div>

<!-- Res Field71 Field -->
<div class="form-group">
    {!! Form::label('res_field71', 'Res Field71:') !!}
    <p>{{ $tXN->res_field71 }}</p>
</div>

<!-- Res Field72 Field -->
<div class="form-group">
    {!! Form::label('res_field72', 'Res Field72:') !!}
    <p>{{ $tXN->res_field72 }}</p>
</div>

<!-- Res Field73 Field -->
<div class="form-group">
    {!! Form::label('res_field73', 'Res Field73:') !!}
    <p>{{ $tXN->res_field73 }}</p>
</div>

<!-- Res Field74 Field -->
<div class="form-group">
    {!! Form::label('res_field74', 'Res Field74:') !!}
    <p>{{ $tXN->res_field74 }}</p>
</div>

<!-- Res Field75 Field -->
<div class="form-group">
    {!! Form::label('res_field75', 'Res Field75:') !!}
    <p>{{ $tXN->res_field75 }}</p>
</div>

<!-- Res Field76 Field -->
<div class="form-group">
    {!! Form::label('res_field76', 'Res Field76:') !!}
    <p>{{ $tXN->res_field76 }}</p>
</div>

<!-- Res Field77 Field -->
<div class="form-group">
    {!! Form::label('res_field77', 'Res Field77:') !!}
    <p>{{ $tXN->res_field77 }}</p>
</div>

<!-- Res Field78 Field -->
<div class="form-group">
    {!! Form::label('res_field78', 'Res Field78:') !!}
    <p>{{ $tXN->res_field78 }}</p>
</div>

<!-- Res Field79 Field -->
<div class="form-group">
    {!! Form::label('res_field79', 'Res Field79:') !!}
    <p>{{ $tXN->res_field79 }}</p>
</div>

<!-- Res Field80 Field -->
<div class="form-group">
    {!! Form::label('res_field80', 'Res Field80:') !!}
    <p>{{ $tXN->res_field80 }}</p>
</div>

<!-- Res Field81 Field -->
<div class="form-group">
    {!! Form::label('res_field81', 'Res Field81:') !!}
    <p>{{ $tXN->res_field81 }}</p>
</div>

<!-- Res Field82 Field -->
<div class="form-group">
    {!! Form::label('res_field82', 'Res Field82:') !!}
    <p>{{ $tXN->res_field82 }}</p>
</div>

<!-- Res Field83 Field -->
<div class="form-group">
    {!! Form::label('res_field83', 'Res Field83:') !!}
    <p>{{ $tXN->res_field83 }}</p>
</div>

<!-- Res Field84 Field -->
<div class="form-group">
    {!! Form::label('res_field84', 'Res Field84:') !!}
    <p>{{ $tXN->res_field84 }}</p>
</div>

<!-- Res Field85 Field -->
<div class="form-group">
    {!! Form::label('res_field85', 'Res Field85:') !!}
    <p>{{ $tXN->res_field85 }}</p>
</div>

<!-- Res Field86 Field -->
<div class="form-group">
    {!! Form::label('res_field86', 'Res Field86:') !!}
    <p>{{ $tXN->res_field86 }}</p>
</div>

<!-- Res Field87 Field -->
<div class="form-group">
    {!! Form::label('res_field87', 'Res Field87:') !!}
    <p>{{ $tXN->res_field87 }}</p>
</div>

<!-- Res Field88 Field -->
<div class="form-group">
    {!! Form::label('res_field88', 'Res Field88:') !!}
    <p>{{ $tXN->res_field88 }}</p>
</div>

<!-- Res Field89 Field -->
<div class="form-group">
    {!! Form::label('res_field89', 'Res Field89:') !!}
    <p>{{ $tXN->res_field89 }}</p>
</div>

<!-- Res Field90 Field -->
<div class="form-group">
    {!! Form::label('res_field90', 'Res Field90:') !!}
    <p>{{ $tXN->res_field90 }}</p>
</div>

<!-- Res Field91 Field -->
<div class="form-group">
    {!! Form::label('res_field91', 'Res Field91:') !!}
    <p>{{ $tXN->res_field91 }}</p>
</div>

<!-- Res Field92 Field -->
<div class="form-group">
    {!! Form::label('res_field92', 'Res Field92:') !!}
    <p>{{ $tXN->res_field92 }}</p>
</div>

<!-- Res Field93 Field -->
<div class="form-group">
    {!! Form::label('res_field93', 'Res Field93:') !!}
    <p>{{ $tXN->res_field93 }}</p>
</div>

<!-- Res Field94 Field -->
<div class="form-group">
    {!! Form::label('res_field94', 'Res Field94:') !!}
    <p>{{ $tXN->res_field94 }}</p>
</div>

<!-- Res Field95 Field -->
<div class="form-group">
    {!! Form::label('res_field95', 'Res Field95:') !!}
    <p>{{ $tXN->res_field95 }}</p>
</div>

<!-- Res Field96 Field -->
<div class="form-group">
    {!! Form::label('res_field96', 'Res Field96:') !!}
    <p>{{ $tXN->res_field96 }}</p>
</div>

<!-- Res Field97 Field -->
<div class="form-group">
    {!! Form::label('res_field97', 'Res Field97:') !!}
    <p>{{ $tXN->res_field97 }}</p>
</div>

<!-- Res Field98 Field -->
<div class="form-group">
    {!! Form::label('res_field98', 'Res Field98:') !!}
    <p>{{ $tXN->res_field98 }}</p>
</div>

<!-- Res Field99 Field -->
<div class="form-group">
    {!! Form::label('res_field99', 'Res Field99:') !!}
    <p>{{ $tXN->res_field99 }}</p>
</div>

<!-- Res Field100 Field -->
<div class="form-group">
    {!! Form::label('res_field100', 'Res Field100:') !!}
    <p>{{ $tXN->res_field100 }}</p>
</div>

<!-- Res Field101 Field -->
<div class="form-group">
    {!! Form::label('res_field101', 'Res Field101:') !!}
    <p>{{ $tXN->res_field101 }}</p>
</div>

<!-- Res Field102 Field -->
<div class="form-group">
    {!! Form::label('res_field102', 'Res Field102:') !!}
    <p>{{ $tXN->res_field102 }}</p>
</div>

<!-- Res Field103 Field -->
<div class="form-group">
    {!! Form::label('res_field103', 'Res Field103:') !!}
    <p>{{ $tXN->res_field103 }}</p>
</div>

<!-- Res Field104 Field -->
<div class="form-group">
    {!! Form::label('res_field104', 'Res Field104:') !!}
    <p>{{ $tXN->res_field104 }}</p>
</div>

<!-- Res Field105 Field -->
<div class="form-group">
    {!! Form::label('res_field105', 'Res Field105:') !!}
    <p>{{ $tXN->res_field105 }}</p>
</div>

<!-- Res Field106 Field -->
<div class="form-group">
    {!! Form::label('res_field106', 'Res Field106:') !!}
    <p>{{ $tXN->res_field106 }}</p>
</div>

<!-- Res Field107 Field -->
<div class="form-group">
    {!! Form::label('res_field107', 'Res Field107:') !!}
    <p>{{ $tXN->res_field107 }}</p>
</div>

<!-- Res Field108 Field -->
<div class="form-group">
    {!! Form::label('res_field108', 'Res Field108:') !!}
    <p>{{ $tXN->res_field108 }}</p>
</div>

<!-- Res Field109 Field -->
<div class="form-group">
    {!! Form::label('res_field109', 'Res Field109:') !!}
    <p>{{ $tXN->res_field109 }}</p>
</div>

<!-- Res Field110 Field -->
<div class="form-group">
    {!! Form::label('res_field110', 'Res Field110:') !!}
    <p>{{ $tXN->res_field110 }}</p>
</div>

<!-- Res Field111 Field -->
<div class="form-group">
    {!! Form::label('res_field111', 'Res Field111:') !!}
    <p>{{ $tXN->res_field111 }}</p>
</div>

<!-- Res Field112 Field -->
<div class="form-group">
    {!! Form::label('res_field112', 'Res Field112:') !!}
    <p>{{ $tXN->res_field112 }}</p>
</div>

<!-- Res Field113 Field -->
<div class="form-group">
    {!! Form::label('res_field113', 'Res Field113:') !!}
    <p>{{ $tXN->res_field113 }}</p>
</div>

<!-- Res Field114 Field -->
<div class="form-group">
    {!! Form::label('res_field114', 'Res Field114:') !!}
    <p>{{ $tXN->res_field114 }}</p>
</div>

<!-- Res Field115 Field -->
<div class="form-group">
    {!! Form::label('res_field115', 'Res Field115:') !!}
    <p>{{ $tXN->res_field115 }}</p>
</div>

<!-- Res Field116 Field -->
<div class="form-group">
    {!! Form::label('res_field116', 'Res Field116:') !!}
    <p>{{ $tXN->res_field116 }}</p>
</div>

<!-- Res Field117 Field -->
<div class="form-group">
    {!! Form::label('res_field117', 'Res Field117:') !!}
    <p>{{ $tXN->res_field117 }}</p>
</div>

<!-- Res Field118 Field -->
<div class="form-group">
    {!! Form::label('res_field118', 'Res Field118:') !!}
    <p>{{ $tXN->res_field118 }}</p>
</div>

<!-- Res Field119 Field -->
<div class="form-group">
    {!! Form::label('res_field119', 'Res Field119:') !!}
    <p>{{ $tXN->res_field119 }}</p>
</div>

<!-- Res Field120 Field -->
<div class="form-group">
    {!! Form::label('res_field120', 'Res Field120:') !!}
    <p>{{ $tXN->res_field120 }}</p>
</div>

<!-- Res Field121 Field -->
<div class="form-group">
    {!! Form::label('res_field121', 'Res Field121:') !!}
    <p>{{ $tXN->res_field121 }}</p>
</div>

<!-- Res Field122 Field -->
<div class="form-group">
    {!! Form::label('res_field122', 'Res Field122:') !!}
    <p>{{ $tXN->res_field122 }}</p>
</div>

<!-- Res Field123 Field -->
<div class="form-group">
    {!! Form::label('res_field123', 'Res Field123:') !!}
    <p>{{ $tXN->res_field123 }}</p>
</div>

<!-- Res Field124 Field -->
<div class="form-group">
    {!! Form::label('res_field124', 'Res Field124:') !!}
    <p>{{ $tXN->res_field124 }}</p>
</div>

<!-- Res Field125 Field -->
<div class="form-group">
    {!! Form::label('res_field125', 'Res Field125:') !!}
    <p>{{ $tXN->res_field125 }}</p>
</div>

<!-- Res Field126 Field -->
<div class="form-group">
    {!! Form::label('res_field126', 'Res Field126:') !!}
    <p>{{ $tXN->res_field126 }}</p>
</div>

<!-- Res Field127 Field -->
<div class="form-group">
    {!! Form::label('res_field127', 'Res Field127:') !!}
    <p>{{ $tXN->res_field127 }}</p>
</div>

<!-- Res Field128 Field -->
<div class="form-group">
    {!! Form::label('res_field128', 'Res Field128:') !!}
    <p>{{ $tXN->res_field128 }}</p>
</div>

<!-- Request Field -->
<div class="form-group">
    {!! Form::label('request', 'Request:') !!}
    <p>{{ $tXN->request }}</p>
</div>

<!-- Response Field -->
<div class="form-group">
    {!! Form::label('response', 'Response:') !!}
    <p>{{ $tXN->response }}</p>
</div>

<!-- Extra Data Field -->
<div class="form-group">
    {!! Form::label('extra_data', 'Extra Data:') !!}
    <p>{{ $tXN->extra_data }}</p>
</div>

<!-- Sync Message Field -->
<div class="form-group">
    {!! Form::label('sync_message', 'Sync Message:') !!}
    <p>{{ $tXN->sync_message }}</p>
</div>

<!-- Need Sending Field -->
<div class="form-group">
    {!! Form::label('need_sending', 'Need Sending:') !!}
    <p>{{ $tXN->need_sending }}</p>
</div>

<!-- Sent Field -->
<div class="form-group">
    {!! Form::label('sent', 'Sent:') !!}
    <p>{{ $tXN->sent }}</p>
</div>

<!-- Received Field -->
<div class="form-group">
    {!! Form::label('received', 'Received:') !!}
    <p>{{ $tXN->received }}</p>
</div>

<!-- Aml Check Field -->
<div class="form-group">
    {!! Form::label('aml_check', 'Aml Check:') !!}
    <p>{{ $tXN->aml_check }}</p>
</div>

<!-- Aml Check Sent Field -->
<div class="form-group">
    {!! Form::label('aml_check_sent', 'Aml Check Sent:') !!}
    <p>{{ $tXN->aml_check_sent }}</p>
</div>

<!-- Aml Check Retries Field -->
<div class="form-group">
    {!! Form::label('aml_check_retries', 'Aml Check Retries:') !!}
    <p>{{ $tXN->aml_check_retries }}</p>
</div>

<!-- Aml Listed Field -->
<div class="form-group">
    {!! Form::label('aml_listed', 'Aml Listed:') !!}
    <p>{{ $tXN->aml_listed }}</p>
</div>

<!-- Posted Field -->
<div class="form-group">
    {!! Form::label('posted', 'Posted:') !!}
    <p>{{ $tXN->posted }}</p>
</div>

<!-- Maker Checker Approve Status Field -->
<div class="form-group">
    {!! Form::label('maker_checker_approve_status', 'Maker Checker Approve Status:') !!}
    <p>{{ $tXN->maker_checker_approve_status }}</p>
</div>

<!-- Maker Checker Reject Status Field -->
<div class="form-group">
    {!! Form::label('maker_checker_reject_status', 'Maker Checker Reject Status:') !!}
    <p>{{ $tXN->maker_checker_reject_status }}</p>
</div>

<!-- Approved At Field -->
<div class="form-group">
    {!! Form::label('approved_at', 'Approved At:') !!}
    <p>{{ $tXN->approved_at }}</p>
</div>

<!-- Rejected At Field -->
<div class="form-group">
    {!! Form::label('rejected_at', 'Rejected At:') !!}
    <p>{{ $tXN->rejected_at }}</p>
</div>

